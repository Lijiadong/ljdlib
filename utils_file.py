...
if __name__ == '__main__':
    ...
    train_path = '/Users/admin/CAIL2021/xxcq/xxcq_small.json'
    with open(train_path, 'r', encoding='utf-8') as f:
        all_f = f.readlines()
        print(len(all_f))
        train, dev = train_test_split(all_f, test_size=0.1,random_state=0,shuffle=True)
        save_obj("./data/train.txt", train)
        save_obj("./data/dev.txt", dev)
        # with open('./data/train.json', 'w', encoding='utf-8') as f:
        #     f.writelines('\n'.join(train))
        # with open('./data/dev.json', 'w', encoding='utf-8') as f:
        #     f.writelines(''.join(train))

    # read_json_to_txt('./data/train.json', "./data/train.txt")
    # read_json_to_txt('./data/dev.json', "./data/dev.txt")
    # main("./xxcq_test.json","./test.txt")
    # main("./xxcq_test.json","./dev.txt")
    # creat_label()
    # check("./data/train.txt")
